...
anime_map_a_converters = {
        ...
        'air_date': lambda x: datetime.date.fromtimestamp(int(x)),
        'end_date': lambda x: datetime.date.fromtimestamp(int(x)),
        'rating': lambda x: int(x)/100,
        'vote_count': int,
        'temp_rating': lambda x: int(x)/100,
        'temp_vote_count': int,
        'average_review_rating': lambda x: int(x)/100,
        'review_count': int,
        'is_18_restricted': lambda x: x=="1",
        'ann_id': int,
        'allcinema_id': int,
        'anidb_updated': lambda x: datetime.datetime.fromtimestamp(int(x)),
        'special_count': int,
        'credit_count': int,
        'other_count': int,
        'trailer_count': int,
...
file_map_f_converters = {
        ...
        'aired_date': lambda x: datetime.date.fromtimestamp(int(x)),
        'mylist_state': lambda x: mylist_state_map[x],
        'mylist_filestate': lambda x: mylist_filestate_map[x],
        'mylist_viewed': lambda x: x == '1',
        'mylist_viewdate': lambda x: datetime.datetime.fromtimestamp(int(x)),
...
episode_map_converters = {
        ...
        'rating': lambda x: int(x)/100,
        'votes': int,
        'aired': lambda x: datetime.date.fromtimestamp(int(x)),
        'type': lambda x: episode_type_map[x],
        }
...
anime_map_a = ['aid','unused','year','type','related_aid_list','related_aid_type','retired','retired',
         'not_implemented','not_implemented','not_implemented','not_implemented','not_implemented','not_implemented','retired','retired',
         'nr_of_episodes','highest_episode_number','special_ep_count','air_date','end_date','url','picname','retired',
         'rating','vote_count','temp_rating','temp_vote_count','average_review_rating','review_count','not_implemented','is_18_restricted',
...
def checkMapFileF(verbos=False):
    getGeneralMap = lambda: getFileMapF()
...
def checkMapFileA(verbos=False):
    getGeneralMap = lambda: getFileMapA()
